//
//  MsgIIjomes.swift
//  kdb3
//
//  Created by Benny Pollak on 11/11/18.
//  Copyright © 2018 Alben Software. All rights reserved.
//

import Foundation

struct MsgIjomes {
    
    static let
    msgIjomes = [
        ["ohno2.png", "Oh no!"]
        , ["oy.jpeg", "Oy vei!"]
        , ["fu.png", "Fuck you!"]
        , ["tu.jpg", "Thank you!"]
        , ["urwelcome.png", "You're welcome!"]
        , ["insult2.jpg", "😤 ###insult"]
        , ["love2.png", "❤️ ###lovism"]
        , ["sad.png", "So fucking sad!"]
        , ["trump2.png", "###trumpism"]
        , ["shakespeare.jpg", "###bard"]
        , ["iwarnyou.png", "I warn you!"]
        , ["sohappy.png", "So happy!"]
        , ["imsick.png", "So sick!"]
        , ["happy.png", "I'm very happy!"]
        , ["ok.jpg", "Ok!"]
        , ["ok.png", "Yes!"]
        , ["thatsweird.jpg", "That's weird!"]
        , ["Imbusy.jpg", "I'm busy!"]
        , ["embarassing.jpg", "That's embarrasing!"]
        , ["370-pretty-large-eyes.png", "Very cute!"]
        , ["waiting.png", "Where the hell are you!"]
        
        
        , ["ahhhhh.jpg", "Ahhh!"]
        , ["truelove.png", "True love!"]
        , ["badday.png", "Having a bad day!"]
        , ["stella.jpg", "Stella!"]
        , ["boobs1.jpg", "Life is beautiful!"]
        , ["angry.jpeg", "I'm very angry!"]
        , ["thumbs_down_angry.png", "No way!"]
        , ["wasntme.png", "It wasn't me!"]
        , ["what.png", "What?"]
        , ["wtf.png", "WTF!"]
        
        , ["dontlike.png", "I don't like that!"]
        , ["happy-smiley.jpg", "That makes me happy"]
        , ["shhh.png", "Quiet!"]
        
        
        , ["sosweet.png", "That's so sweet!"]
        , ["gotcha.png", "Gotcha!"]
        , ["embarassing.jpg", "That's embarrasing"]
        
        , ["doh.jpeg", "D'oh!"]
        , ["tu.jpg", "Thank you!"]
        , ["sleeping.jpg", "I'm sleeping"]
        
        , ["welcome.jpg", "Welcome!"]
        , ["tired.jpg", "I'm beat!"]
        , ["tp.jpg", "What? I'm busy"]
        , ["yell.jpg", "WHAT?!"]
        , ["timeout.jpg", "Timeout!"]
        , ["goodluck.gif", "Good luck!"]
        , ["right-on.png", "Right on!"]
        , ["watching-you.png", "I'm watching you!"]
        , ["goingtosleep.png", "I'm going to sleep"]
        , ["cookoo.jpg", "You're crazy"]
        , ["cryer.jpg", "Stop crying!"]
        , ["studying.jpg", "I'm studying"]
        , ["pray.jpg", "Please, God!"]
        , ["notaword.jpg", "Not a word"]
        , ["horror.jpg", "Horror!"]
        , ["scram.png", "Scram!"]
        , ["no.jpg", "No!"]
        , ["dont-speak-smiley.png", "Oh shit!"]
        , ["cry.jpg", "Waaaaaah!"]
        , ["Dreammmmyyyy.jpg", "Dreammmmyyyy!"]
        , ["Nahnahnahnahnahnah.jpg", "Nahnahnahnahnahnah!"]
        , ["Einstein_tongue.jpg", "Yes, but I'm smarter than you!"]
                , ["empty-toilet-paper-roll.jpg", "Oh, crap!"]
                , ["notagain.gif", "Not again!"]
                , ["imhungry.jpg", "I was very hungry!"]
        //        , ["", ""]
    ]
}
